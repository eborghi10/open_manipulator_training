...
class SmartGrasper(object):
    ...
    __current_model_name = "standard_sphere_blue"
    ...
    def __init__(self):
        ...
        # Gripper topic
        self.gripper_pub = rospy.Publisher("/gripper_position/command", Float64, queue_size=1)
        ...
        if self.__arm_traj_client.wait_for_server(timeout=rospy.Duration(4.0)) is False:
            rospy.logfatal("Failed to connect to /arm_controller/follow_joint_trajectory in 4 sec.")
            raise Exception("Failed to connect to /arm_controller/follow_joint_trajectory in 4 sec.")
    ...
    def get_object_pose(self, object_name):
        ...
        """
        return self.__get_pose_srv.call(object_name, "world").pose
    ...
    def move_tip_absolute(self, target):
        ...
        """
        # rospy.loginfo("Target pose: {}".format(target))
        self.debug_pose(target)
        self.arm_commander.set_start_state_to_current_state()
        self.arm_commander.set_pose_target(target)
    ...
    def move_tip(self, x=0., y=0., z=0., roll=0., pitch=0., yaw=0.):
        ...
        return self.move_tip_absolute(final_pose)
    ...
    def open_hand(self):
        ...
        self.gripper_pub.publish(Float64(0.2))
    ...
    def close_hand(self):
        ...
        self.gripper_pub.publish(Float64(0))
    ...
    def check_fingers_collisions(self, enable=True):
        ...
        for object_name in objects:
            if object_name not in acm.entry_names:
                rospy.loginfo("Adding '{}' to allowed collision matrix".format(object_name))
        ...
        for index_entry_values, entry_values in enumerate(acm.entry_values):
            if "gripper_link" in acm.entry_names[index_entry_values]:
                ...
                for index_value, _ in enumerate(entry_values.enabled):
                    if "gripper_link" in acm.entry_names[index_value]:
                        if enable:
                            acm.entry_values[index_entry_values].enabled[index_value] = False
                        else:
                            acm.entry_values[index_entry_values].enabled[index_value] = True
    ...
    def debug_pose(self, pose):
        ...
        pub = rospy.Publisher("odom", Odometry, queue_size=1, latch=True)
        msg = Odometry()
    ...
    def pick(self):
        ...
        self.send_command(self.__zero_pose, duration=3.0)
        # self.move_tip_absolute(self.__zero_pose)

        ball_pose = self.get_object_pose("standard_sphere_blue")
        # ball_pose.position.z += 0.2

        self.__grasp(ball_pose)

        while not rospy.is_shutdown():
            pass

        # setting an absolute orientation (from the top)
        # The gripper will be looking down
        ...
        if not self.move_tip_absolute(ball_pose):
            # raise Exception("Plan to the ball not found")
            pass

        rospy.loginfo("Grasping")
        ball_pose.position.z = 0.15
        ...
        intermmediate_pose = self.__zero_pose
        intermmediate_pose['joint1'] = pi/2
        self.send_command(intermmediate_pose, duration=1.5)
        time.sleep(0.1)
        rospy.loginfo("Leaving the ball")
        self.move_tip(x=-0.075, # Forward (direction of the plates)
                      z=0.1,  # Down closer to the plates
                      roll=pi/2)
        rospy.sleep(1)
        self.open_hand()
    ...
    def __grasp(self, arm_target):
        ...
        arm_above_ball.position.z += 0.1#-= 0.12
